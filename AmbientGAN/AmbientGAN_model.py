...
from AmbientGAN.AmbientGAN_Algorithms import AmbientGAN_Generator, AmbientGAN_Standard_Discriminator, AmbientGANInputPreparation
...
def save_model_parameters(params, elapsed_time, fold_save_path):
    """
    Save the model parameters and elapsed training time (in seconds) to a CSV file in the given fold directory.
    """
...
# Helper function to save the parameter counts of a model (including the total count).
def save_model_parameter_counts(model, csv_path):
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        for p in param_list:
            writer.writerow(p)
        # Append a final row with the total count.
...
class AmbientGAN:
    def __init__(self, sparse_tensor, dense_tensor, parameters, rank_similarity_tensors, args, sparsity_level):
        ...
        self.alpha = 0.5  # 1
        self.h_dim = 6
        self.z_dim = 100  # noise dimension
    ...
    def train_step(self, train_batch, generator, discriminator, g_optimizer, d_optimizer):
        ...
        # Generate AmbientGAN input.
        preparation = AmbientGANInputPreparation(self.z_dim, self.h_dim)
        combined_input = preparation([train_set_tensor, tf.shape(train_set_tensor)[0]])
        noise, cont_code1 = combined_input
        
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            # Generator produces its output.
            G_sample = generator(combined_input, training=True)
            # Use the mask to combine observed values with generator output.
            fake_data = mask_train_x * train_set_tensor + (1 - mask_train_x) * G_sample
            
            # Apply measurement operator to the imputed result.
            measured_fake_data = apply_measurement(fake_data)
            # Also apply measurement operator to the original (observed) real data.
            measured_real_data = apply_measurement(train_set_tensor)
            
            d_fake = discriminator(measured_fake_data, training=True)
            d_real = discriminator(measured_real_data, training=True)
    ...
    def train_and_evaluate_fold(self, fold, train_index, val_index, early_stopping_patience):
        ...
        # Convert tensors and adjust dimensions.
        ...
        generator_global = AmbientGAN_Generator(
            channel=train_sparse.shape[-1],
            output_size=output_size,
            input_shape=input_shape
        )
        discriminator_global = AmbientGAN_Standard_Discriminator(self.h_dim)
        ...
        for epoch in tqdm(range(self.global_max_iter), desc=f"Fold {fold+1} Training Progress"):
            ...
            for val_batch in val_dataset:
                ...
                measured_pred = apply_measurement(pred_val)
                val_generated_data.append(measured_pred)
        ...
        # Save model parameter counts (with total count) for generator and discriminator.
    ...
    # -------------------------------
    # Global Training with K-Fold Cross Validation
    # -------------------------------
