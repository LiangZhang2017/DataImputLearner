...
import pandas as pd
import numpy as np
import json  # for saving mappings to JSON



def get_id_mappings(data, student_col='Anon Student Id', step_col='Step Name'):
    """
    Build dictionaries that map each unique student and step (question)
    to a numeric ID.
    """
    # Create a categorical series for students
    student_cat = data[student_col].astype('category')
    ...
    # Create a categorical series for steps (questions)
    step_cat = data[step_col].astype('category')
    step_categories = step_cat.cat.categories
    step_mapping = {step_name: code for code, step_name in enumerate(step_categories)}
    
    return student_mapping, step_mapping

def preprocess_mathia_data(base_filename, input_data_dir):
    """
...
if __name__ == '__main__':
    ...
    # All base files: 
    # scale_drawings_3, 
    # Analyzing_Models_of_Two-Step_Linear_Relationships_2019-2020, 
    # Modeling_Two-Step_Expressions_2019-2020_v2
    # Using_Scale_Factor_2019-2020
    # worksheet_grapher_a1_patterns_2step_expr
    # -------------------------------
    '''
    ...
    base_filename = "Analyzing_Models_of_Two-Step_Linear_Relationships_2019-2020"
    input_data_dir = os.path.join(os.getcwd(), "dataset", "source_dataset", "MATHia")
    ...
    # -------------------------------
    data, student_mapping, step_mapping = preprocess_mathia_data(base_filename, input_data_dir)
    if data is None:
