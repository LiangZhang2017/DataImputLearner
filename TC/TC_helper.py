...
def get_missing_indices(tensor):
    """
    Returns the indices of the missing (NaN) entries in the tensor.

    Args:
        tensor (tf.Tensor or np.ndarray): Input tensor with missing values represented as np.nan.
    
    Returns:
        missing_indices (np.ndarray): Array of indices (each row is a coordinate) where the tensor is NaN.
    """
    # Convert to NumPy array if tensor is a TF tensor.
    tensor_np = tensor.numpy() if isinstance(tensor, tf.Tensor) else tensor
    missing_indices = np.array(np.where(np.isnan(tensor_np))).T
...
def save_indices(indices, max_iter, mode, model_name, lesson_id, sparsity_level, fold, fold_save_path):
    indices_file_path = os.path.join(fold_save_path,
                             f"{model_name}_{lesson_id}_{sparsity_level}_{mode}_MaxIter{max_iter}_fold{fold}_indices.npy")

    np.save(indices_file_path, indices)

def save_tensor(tensor, max_iter, mode, model_name, lesson_id, sparsity_level, fold, fold_save_path):
    # Convert tensor to NumPy array if it's not already
    tensor_np = tensor.numpy() if not isinstance(tensor, np.ndarray) else tensor

    # Define the file path for saving the data
    data_file_path = os.path.join(fold_save_path,
                                  f"{model_name}_{lesson_id}_{sparsity_level}_{mode}_MaxIter{max_iter}_fold{fold}.npy")
...
def save_metrics(train_perf, max_iter, mode, model_name, lesson_id, sparsity_level, fold, fold_save_path):
    # Define CSV file path.
    csv_file = os.path.join(
        fold_save_path,
        f"{model_name}_{lesson_id}_{sparsity_level}_{mode}_MaxIter{max_iter}_fold{fold}_metrics.csv"
    )
    # Define header (adjust as needed).
    header = ["Epoch", "MAE", "RMSE", "RSE", "AUC", "Cross_Entropy"]
    
    # Write the metrics to the CSV file.
    with open(csv_file, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(header)
        for i, row in enumerate(train_perf, start=1):
            writer.writerow([i] + row)
            
    print(f"Saved metrics to CSV file: {csv_file}")
